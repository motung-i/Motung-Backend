...
import kr.motung_i.backend.testfixture.ReviewFixture.createTestFetchDetailReviewsRequest
...
class FetchDetailReviewsUsecaseImplTest {
    @Mock
    private lateinit var reviewRepository: ReviewRepository

    @InjectMocks
    private lateinit var fetchDetailReviewsUsecaseImpl: FetchDetailReviewsUsecaseImpl

    @Test
    @DisplayName("리뷰를 조회하면 올바른 DTO 형식으로 반환한다")
    fun 리뷰를_조회하면_올바른_DTO_형식으로_반환한다() {
        val request = createTestFetchDetailReviewsRequest()
        ...
        val result = fetchDetailReviewsUsecaseImpl.execute(request)
